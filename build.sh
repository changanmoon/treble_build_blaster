...
echo
echo "--------------------------------------"
echo "    PixelBlaster OS 13.0 Buildbot     "
echo "                  by                  "
echo "              changanmoon             "
echo "--------------------------------------"
echo
...
initRepos() {
    if [ ! -d .repo ]; then
        ...
        cp $BL/manifest.xml .repo/local_manifests/blaster.xml
        echo
    fi
}
...
generatePackages() {
    ...
    xz -cv $BD/system-treble_arm64_bvN.img -T0 > $BD/PixelBlaster_arm64-ab-13.0-$buildDate-UNOFFICIAL.img.xz
    xz -cv $BD/system-treble_arm64_bvN-vndklite.img -T0 > $BD/PixelBlaster_arm64-ab-vndklite-13.0-$buildDate-UNOFFICIAL.img.xz
    xz -cv $BD/system-treble_arm64_bvN-slim.img -T0 > $BD/PixelBlaster_arm64-ab-slim-13.0-$buildDate-UNOFFICIAL.img.xz
    rm -rf $BD/system-*.img
    echo
}
...
generateOta() {
    ...
    find $BD/ -name "PixelBlaster_*" | sort | {
        while read file; do
            ...
            url="https://github.com/changanmoon/treble_build_blaster/releases/download/$version/$filename"
